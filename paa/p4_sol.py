...
def main():
    ...
    with open("paa/flights_dataset.txt", "r+") as FILE:
        for line in FILE:
            line = line.split(" ")
            flight = line[0]
            ...
            for i in range(2, len(line) - 2):
                if line[i][0].isalpha():    
                    ...
                    time = time_difference(flight_object.get_takeoff_time(), line[i + 1])
                    ...
                    for f in fs:
                        ...
                        if time >= 30:
                            ...
                            time = time_difference(f.get_takeoff_time(), flight_intermediate_origin.get_arrival_time())
                            if time == 0:
                                time = 1440
                            graph.add_edge(flight_intermediate_origin, f, time)
    ...
    if dijkstra(graph, origin, destination):
        ...
        distance_in_hours = f"{distance // 60}:{distance % 60}"
        print(f"Distancia: {destination.get_distance()} / {distance_in_hours}")

        flight_list = {}
        get_flight_path(graph, origin, destination, flight_list)
        
        for flight in flight_list:
            print(flight_list[flight], end="")
    ...
    if v == s:
        ...
        if isinstance(v, Flight):
            if v.get_type() == "Origem":
                ...
                if v.get_label() in flight_list.keys():
                    flight_list[v.get_label()] = flight_list[v.get_label()] + f"\n{v.get_label()} {v.get_takeoff_time()}"
                else:
